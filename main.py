...
def ffprobe_duration(path: Path) -> float:
    ...
    if not match:
        raise RuntimeError(f"Could not read video duration. ffmpeg output: {result.stderr[-1000:]}")
...
async def fast_download(message, destination: Path, status_message) -> None:
    ...
    logger.info("Fast download started: %.1f MB", file_size / 1024 / 1024)
    progress = {"current": 0}
    ...
    logger.info("Fast download complete: %.1f MB in %.1fs (%.2f MB/s)", written / 1024 / 1024, elapsed, speed)
    try:
        await status_message.edit(f"✅ Download முடிந்தது — {written / 1024 / 1024:.1f} MB in {elapsed:.1f}s")
    except Exception:
        pass
...
def make_chunk(src: Path, dst: Path, start: float, duration: float) -> None:
    ...
    subprocess.run(
        [
            ...
            "-preset", "ultrafast", "-crf", "24", "-threads", "0",
            "-c:a", "aac", "-b:a", "128k", "-movflags", "+faststart", str(dst),
        ], check=True, timeout=900,
    )
    elapsed = time.monotonic() - started
    size_mb = dst.stat().st_size / 1024 / 1024
    logger.info("Part created: %.1fs, %.2f MB, %.1fs processing time", duration, size_mb, elapsed)


def send_via_bot_api(chat_id: int, output: Path, caption: str, has_next: bool, duration: float) -> None:
    keyboard = None
    if has_next:
        keyboard = {"inline_keyboard": [[{"text": "▶️ அடுத்து", "callback_data": "NEXT"}]]}
    data = {
        "chat_id": str(chat_id),
        "caption": caption,
    ...
    if size_mb >= 49:
        raise RuntimeError(f"Part is too large for Bot API upload: {size_mb:.2f} MB")
    started = time.monotonic()
    ...
    if not response.ok:
        raise RuntimeError(f"Telegram Bot API HTTP {response.status_code}: {response.text[-1000:]}")
    payload = response.json()
    if not payload.get("ok"):
        raise RuntimeError(f"Telegram Bot API error: {payload}")
    logger.info("Bot API upload complete: %.2f MB in %.1fs", size_mb, time.monotonic() - started)
...
async def process_and_send(client: TelegramClient, user_id: int, index: int):
    ...
    await asyncio.to_thread(send_via_bot_api, user_id, output, caption, index + 1 < session["total"], duration)
...
async def video_handler(event):
    ...
    try:
        ...
        sessions[user_id] = {"dir": temp_dir, "source": str(source), "duration": duration, "total": total, "index": 0}
...
async def main():
    start_health_server()
    if BOT_ROLE == "standby":
        logger.info("BOT_ROLE=standby; Telegram polling disabled on this deployment")
        while True:
